...
/** Dish + ingredient/equipment names in both languages so DB matching can try each. */
...
{"dish": {"th": "ชื่อเมนูภาษาไทย", "en": "Dish name in English"} | null,
 ...
 "equipment": [{"th": "ชื่ออุปกรณ์ไทย", "en": "Equipment in English"}]}
Rules:
...
- "equipment": cooking equipment visible in the photo, plus what the dish typically requires (pot, pan, wok, oven, steamer, blender, grill...). Utensils for eating (spoon, plate, bowl for serving) excluded. Max 6.
...
export class VisionService {
  ...
  async analyzeFood(image: File): Promise<DishAnalysis> {
    ...
    return {
      dish: dish && (dish.th || dish.en) ? dish : null,
      ingredients: names(parsed.ingredients, 12),
      equipment: names((parsed as Record<string, unknown>).equipment, 6),
    };
  }
}
