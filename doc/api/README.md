...
**Postman collection**: [doc/postman/too-yen-api.postman_collection.json](../postman/too-yen-api.postman_collection.json) — generated from the live `/swagger/json` OpenAPI doc, grouped by module, with `{{base_url}}`/`{{access_token}}` collection variables. Run **Auth → Log in** first — a test script auto-saves `access_token` for every other request. Regenerate after route/DTO changes: `bun run dev`, then re-run the export (ask Claude, or `curl localhost:3000/swagger/json`).

**Error codes**: [error-codes.md](error-codes.md) — every `error.code` the API can return, grouped by module, generated by grepping the actual `throw` sites (not hand-maintained). Swagger only documents the `2xx` shape per endpoint (error mapping is a global handler in `src/index.ts`, not per-route), so this doc is the only place the UI team can see which codes to branch on.
